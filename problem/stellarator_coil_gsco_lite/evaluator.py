...
class SimpleGSCOEvaluator:
    ...
    def __init__(self, config):
        ...
        hard_cfg = config.get("gsco_hard", None)
        if not isinstance(hard_cfg, dict):
            hard_cfg = {}
        self.hard_enabled = bool(hard_cfg.get("enabled", False))
        self.hard_f_B_max = hard_cfg.get("f_B_max")
        self.hard_f_S_max = hard_cfg.get("f_S_max")
        self.hard_I_max_max = hard_cfg.get("I_max_max")
    ...
    def evaluate(self, items):
        ...
        for item in items:
            try:
                ...
                if is_forbidden:
                    is_feasible = 0.0
                    cv = 1e6
                    sim_message = "forbidden_cell"
                elif self.hard_enabled:
                    cv = 0.0
                    parts = []
                    if self.hard_f_B_max is not None and float(f_B) > float(self.hard_f_B_max):
                        cv += float(f_B) - float(self.hard_f_B_max)
                        parts.append("f_B")
                    if self.hard_f_S_max is not None and float(f_S) > float(self.hard_f_S_max):
                        cv += float(f_S) - float(self.hard_f_S_max)
                        parts.append("f_S")
                    if self.hard_I_max_max is not None and float(I_max) > float(self.hard_I_max_max):
                        cv += float(I_max) - float(self.hard_I_max_max)
                        parts.append("I_max")
                    is_feasible = 1.0 if cv <= 0.0 else 0.0
                    sim_message = "" if is_feasible >= 1.0 else ("hard_constraint_fail:" + ",".join(parts))
                else:
                    is_feasible = 1.0
                    cv = 0.0
                    sim_message = ""

                constraint_results_list.append({
        ...
        for idx, item in enumerate(items):
            ...
            results['constraint_results'] = constraint_results_list[idx]
