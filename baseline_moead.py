...
class SACSProblem(Problem):
    ...
    def __init__(self, reward_system, config, item_factory, eval_logger=None, pop_size=None):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        evaluated_items, _ = self.reward_system.evaluate(items)
        dt = time.time() - t0
        ...
        if self.eval_logger is not None and evaluated_items:
            ...
            try:
                if self.pop_size and self.pop_size > 0:
                    gen_val = int(start_eval // self.pop_size)
            except Exception:
                gen_val = -1
            try:
                self.eval_logger.log_batch(evaluated_items, generation=gen_val, total_time_sec=dt, tier="true")
            except Exception:
                pass
...
def main():
    ...
    config_data['protocol'].setdefault('algo_id', config.get('protocol.algo_id', None) or 'moead')
    ...
    if len(initial_population_strs) < pop_size:
        initial_population_strs = list(initial_population_strs) + random.choices(
            list(initial_population_strs), k=pop_size - len(initial_population_strs)
        )
    initial_population_strs = list(initial_population_strs)[:pop_size]

    # --- 4. Setup Pymoo Algorithm ---
    problem = SACSProblem(reward_system, config, item_factory, eval_logger=eval_logger, pop_size=pop_size)
    ...
    # NOTE: In pymoo MOEA/D the population size is tightly coupled to the number of reference
    # directions. If these counts mismatch, MOEA/D can crash with out-of-bounds neighbor indices.
    n_obj = len(config.get('goals'))

    def _num_ref_dirs(n_partitions: int, n_obj_: int) -> int:
        return math.comb(n_partitions + n_obj_ - 1, n_obj_ - 1)

    n_partitions = int(config.get('baseline_moead.n_partitions', 12))
    while _num_ref_dirs(n_partitions, n_obj) < pop_size:
        n_partitions += 1

    ref_dirs_full = get_reference_directions("uniform", n_obj, n_partitions=n_partitions)
    if len(ref_dirs_full) > pop_size:
        idx = np.linspace(0, len(ref_dirs_full) - 1, pop_size)
        idx = np.round(idx).astype(int)
        # ensure exact length pop_size (linspace+round can create duplicates)
        idx_unique = []
        seen = set()
        for i in idx:
            ii = int(i)
            if ii not in seen:
                ...
                seen.add(ii)
        if len(idx_unique) < pop_size:
            for ii in range(len(ref_dirs_full)):
                if ii not in seen:
                    idx_unique.append(ii)
                    seen.add(ii)
                if len(idx_unique) == pop_size:
                    break
        ref_dirs = ref_dirs_full[np.array(idx_unique, dtype=int)]
    else:
        ref_dirs = ref_dirs_full

    n_neighbors = int(config.get('baseline_moead.n_neighbors', 15))
    n_neighbors = max(1, min(n_neighbors, len(ref_dirs) - 1))
