...
class BaselineOptimizer:
    def __init__(self, config_data, seed, algo_name):
        ...
        try:
            ...
            problem_id = self.config.get('protocol.problem_id', 'stellarator_coil_gsco_lite')
            algo_id = self.config.get('protocol.algo_id', None) or str(self.algo_name)

            config_data['protocol'].setdefault('problem_id', problem_id)
            config_data['protocol'].setdefault('algo_id', algo_id)
            config_data['logging']['results_base_dir'] = os.path.join('./results', str(problem_id))

            ts = datetime.utcnow().strftime('%Y%m%dT%H%M%S')
            run_name = self.config.get('exper_name', 'run')
            run_id = f"{run_name}_seed{seed}_{ts}"
            run_dir = os.path.join(config_data['logging']['results_base_dir'], algo_id, run_id)
            self.eval_logger = EvalLogger(run_dir, problem_id, algo_id, seed, run_id, config_data, self.goals)
        except Exception:
            self.eval_logger = None
    ...
    def _finalize_protocol(self):
        try:
            if self.eval_logger is not None:
                ...
                _repair_run(Path(self.eval_logger.run_dir))
        except Exception:
            pass
    ...
    def _eval_population(self, items):
        ...
        if self.eval_logger is not None and items:
            try:
                generation = -1
                if getattr(self, 'pop_size', None):
                    if len(self.all_mols) >= 0 and int(self.pop_size) > 0:
                        generation = int(len(self.all_mols) // int(self.pop_size))
                self.eval_logger.log_batch(items, generation=generation, total_time_sec=dt, tier="true")
            except Exception:
                pass
        return items
...
class GreedySearch(BaselineOptimizer):
    def __init__(self, config_data, seed, lambda_S=1e-6):
        super().__init__(config_data, seed, 'GreedySearch')
