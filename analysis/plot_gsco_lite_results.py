...
ALGO_FILES: Dict[str, Dict[int, Path]] = {
    ...
    "OpenEvolve": {
        42: RESULTS_DIR / "baselines" / "OpenEvolve_42_metrics.json",
        43: RESULTS_DIR / "baselines" / "OpenEvolve_43_metrics.json",
    },
}
    ...
    for algo, seed_map in ALGO_FILES.items():
        for seed, path in seed_map.items():
            ...
            out.setdefault(seed, {})[algo] = {
                ...
                "hypervolume": float(max(ys_hv) if ys_hv else 0.0),
            }
    return out
