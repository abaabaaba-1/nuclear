...
def find_runs(results_dir: Path, seeds: list[int], max_eval: int) -> list[RunChoice]:
    ...
    for seed in seeds:
        baseline_candidates = sorted(results_dir.glob(f"*Phase4Hard*Baseline*seed{seed}_*"))
        llm_candidates = sorted(results_dir.glob(f"*Phase4Hard*LLM*seed{seed}_*"))
...
def _plot_multiseed(out_png: Path, title: str, xs: list[float], b_hv: np.ndarray, l_hv: np.ndarray, b_t1: np.ndarray, l_t1: np.ndarray, b_fr: np.ndarray, l_fr: np.ndarray) -> None:
    ...
    for i in range(b_hv.shape[0]):
        ax_hv.plot(x, b_hv[i], color="C0", alpha=0.25, linewidth=1.0)
        ax_hv.plot(x, l_hv[i], color="C1", alpha=0.25, linewidth=1.0)
        ax_t1.plot(x, b_t1[i], color="C0", alpha=0.25, linewidth=1.0)
        ax_t1.plot(x, l_t1[i], color="C1", alpha=0.25, linewidth=1.0)
        ax_fr.plot(x, b_fr[i], color="C0", alpha=0.25, linewidth=1.0)
        ax_fr.plot(x, l_fr[i], color="C1", alpha=0.25, linewidth=1.0)

    def mean_std(a: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        return a.mean(axis=0), a.std(axis=0)

    b_hv_m, b_hv_s = mean_std(b_hv)
    l_hv_m, l_hv_s = mean_std(l_hv)
    b_t1_m, b_t1_s = mean_std(b_t1)
    l_t1_m, l_t1_s = mean_std(l_t1)
    b_fr_m, b_fr_s = mean_std(b_fr)
    l_fr_m, l_fr_s = mean_std(l_fr)

    ax_hv.plot(x, b_hv_m, color="C0", linewidth=2.2, label="Baseline (mean±std)")
    ax_hv.fill_between(x, b_hv_m - b_hv_s, b_hv_m + b_hv_s, color="C0", alpha=0.15)
    ax_hv.plot(x, l_hv_m, color="C1", linewidth=2.2, label="LLM-SemOp (mean±std)")
    ax_hv.fill_between(x, l_hv_m - l_hv_s, l_hv_m + l_hv_s, color="C1", alpha=0.15)

    ax_t1.plot(x, b_t1_m, color="C0", linewidth=2.2, label="Baseline (mean±std)")
    ax_t1.fill_between(x, b_t1_m - b_t1_s, b_t1_m + b_t1_s, color="C0", alpha=0.15)
    ax_t1.plot(x, l_t1_m, color="C1", linewidth=2.2, label="LLM-SemOp (mean±std)")
    ax_t1.fill_between(x, l_t1_m - l_t1_s, l_t1_m + l_t1_s, color="C1", alpha=0.15)

    ax_fr.plot(x, b_fr_m, color="C0", linewidth=2.2, label="Baseline (mean±std)")
    ax_fr.fill_between(x, b_fr_m - b_fr_s, b_fr_m + b_fr_s, color="C0", alpha=0.15)
    ax_fr.plot(x, l_fr_m, color="C1", linewidth=2.2, label="LLM-SemOp (mean±std)")
    ax_fr.fill_between(x, l_fr_m - l_fr_s, l_fr_m + l_fr_s, color="C1", alpha=0.15)
...
def main() -> None:
    ...
    p.add_argument("--results-dir", type=str, default="results/stellarator_coil_gsco_lite/fusionopt_v1")
    p.add_argument("--out-dir", type=str, default="analysis_outputs/phase5/figure")
    ...
    l_hv_rows: list[np.ndarray] = []
    b_t1_rows: list[np.ndarray] = []
    l_t1_rows: list[np.ndarray] = []
    b_fr_rows: list[np.ndarray] = []
    l_fr_rows: list[np.ndarray] = []

    per_seed: dict = {}
    for ch in choices:
        b = compute_series(ch.baseline_dir, max_eval=max_eval, step=step)
        l = compute_series(ch.llm_dir, max_eval=max_eval, step=step)

        if xs_ref is None:
            xs_ref = list(b[0])
        if xs_ref != list(b[0]) or xs_ref != list(l[0]):
            raise ValueError("Inconsistent x-steps across runs; use a common step/max-eval")

        b_hv_rows.append(np.array(b[1], dtype=float))
        l_hv_rows.append(np.array(l[1], dtype=float))
        b_t1_rows.append(np.array(b[2], dtype=float))
        l_t1_rows.append(np.array(l[2], dtype=float))
        b_fr_rows.append(np.array(b[3], dtype=float))
        l_fr_rows.append(np.array(l[3], dtype=float))

        per_seed[str(ch.seed)] = {
            "baseline_dir": str(ch.baseline_dir),
            "llm_dir": str(ch.llm_dir),
        }
    ...
    l_hv = np.stack(l_hv_rows, axis=0)
    b_t1 = np.stack(b_t1_rows, axis=0)
    l_t1 = np.stack(l_t1_rows, axis=0)
    b_fr = np.stack(b_fr_rows, axis=0)
    l_fr = np.stack(l_fr_rows, axis=0)

    out_dir.mkdir(parents=True, exist_ok=True)

    out_png = out_dir / f"gsco_phase4hard_baseline_vs_llm_budget{max_eval}_seeds{min(seeds)}-{max(seeds)}.png"
    title = f"GSCO-Lite Phase4Hard (budget={max_eval}, seeds={','.join(map(str, seeds))})"
    _plot_multiseed(out_png, title, xs_ref, b_hv, l_hv, b_t1, l_t1, b_fr, l_fr)

    out_json = out_dir / f"gsco_phase4hard_baseline_vs_llm_budget{max_eval}_seeds{min(seeds)}-{max(seeds)}.json"
    payload = {
